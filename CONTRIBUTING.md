...
- `duckdb-neo` provides an ergonomic wrapper around the v2 C API. The v2 C API is still in rapid development, so this crate is experimental. Its `capi-v2-p2` feature targets API surface the bundled DuckDB does not provide yet and does not compile.
...
├── duckdb_v2.h        # Only with `capi-v2` + `buildtime_bindgen` (duckdb-neo)
├── libduckdb.so       # Linux
...
```shell
cargo test -p duckdb-neo --features bundled -- --nocapture
```
...
cargo +nightly test --lib --tests --features bundled \
  --target x86_64-unknown-linux-gnu --package duckdb --package duckdb-neo
```
