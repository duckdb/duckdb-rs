...
impl FlatVector {
    ...
    pub fn logical_type(&self) -> LogicalTypeHandle {
        LogicalTypeHandle::from(unsafe { duckdb_vector_get_column_type(self.handle.ptr) })
    }
...
impl StructVector {
    ...
    pub fn child_name(&self, idx: usize) -> DuckDbStr {
        let logical_type = self.logical_type();
        unsafe { DuckDbStr::from_ptr(duckdb_struct_type_child_name(logical_type.ptr, idx as u64)) }
    }
...
impl UnionVector {
    ...
    pub fn member_name(&self, idx: usize) -> DuckDbStr {
        let logical_type = self.logical_type();
        unsafe { DuckDbStr::from_ptr(duckdb_union_type_member_name(logical_type.ptr, idx as u64)) }
    }
}
...
impl Vector {
    ...
    pub fn from_arrow_schema<'b>(schema: &SchemaRef, data_chunk_handle: Arc<DataChunkHandle>) -> Vec<Self> {
        let get_capacity = {
            let data_chunk_handle = data_chunk_handle.clone();
            Arc::new(move || data_chunk_handle.len().max(DataChunkHandle::max_capacity()))
        };
