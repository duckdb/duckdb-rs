...
use super::{column_info::ColumnInfo, logical_type, LogicalTypeHandle};
use crate::{
...
impl DataChunkHandle {
    ...
    /// Create a new [DataChunk] with the given [LogicalTypeHandle]s.
    ...
    /// Get all the [LogicalTypes] in this [DataChunk].
    pub fn get_logical_types(&self) -> Vec<LogicalType> {
        let num_columns = self.num_columns();
        (0..num_columns)
            .map(|idx| unsafe {
                let vector = duckdb_data_chunk_get_vector(self.ptr, idx as u64);
                let handle = LogicalTypeHandle::from(duckdb_vector_get_column_type(vector));
