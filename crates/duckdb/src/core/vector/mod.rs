...
//! traversal invariants. The public vector-family types are family-specific
...
pub(in crate::core) use lease::{BorrowGuard, BorrowRegistry};
#[cfg(test)]
use resize::{MAX_VECTOR_SIZE, max_resize_data_width};
#[cfg(test)]
pub(super) use state::UNDER_CONSTRUCTION_MESSAGE;
...
impl<'a> VectorRef<'a> {
    ...
    pub(super) unsafe fn shared_view(ptr: duckdb_vector, capacity: usize, state: &'a VectorStateCell) -> Result<Self> {
    ...
    pub(super) fn ensure_writable(&self) -> Result<()> {
        if self.access == VectorAccess::Writable {
            ...
            Err(duckdb_failure_from_message("DuckDB vector is read-only"))
        }
    }
    ...
    fn check_variable_length_insert(&self, index: usize) -> Result<()> {
        self.ensure_writable()?;
        let actual = self.logical_type.id();
        if !matches!(
            actual,
            LogicalTypeId::Varchar | LogicalTypeId::Blob | LogicalTypeId::Geometry
        ) {
            return Err(duckdb_failure_from_message(format!(
                "cannot insert variable-length data into {actual:?} vector"
            )));
        }
        self.check_index(index, "row index")
    }
