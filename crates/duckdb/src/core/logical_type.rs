...
impl ListLogicalType {
    pub fn child_type(&self) -> LogicalType {
        let handle = LogicalTypeHandle::new(unsafe { duckdb_list_type_child_type(self.0.ptr) });
...
impl UnionLogicalType {
    ...
    pub fn member_type(&self, idx: usize) -> LogicalType {
        let handle = LogicalTypeHandle::new(unsafe { duckdb_union_type_member_type(self.0.ptr, idx as u64) });
        LogicalType::from(handle)
    }
...
impl StructLogicalType {
    ...
    pub fn child_type(&self, idx: usize) -> LogicalType {
        let handle = LogicalTypeHandle::new(unsafe { duckdb_struct_type_child_type(self.0.ptr, idx as u64) });
        LogicalType::from(handle)
    }
...
impl MapLogicalType {
    pub fn key_type(&self) -> LogicalType {
        let handle = LogicalTypeHandle::new(unsafe { duckdb_map_type_key_type(self.0.ptr) });
        handle.into()
    }

    pub fn value_type(&self) -> LogicalType {
        let handle = LogicalTypeHandle::new(unsafe { duckdb_map_type_value_type(self.0.ptr) });
        handle.into()
    }
}
...
impl ArrayLogicalType {
    ...
    pub fn child_type(&self) -> LogicalType {
        let handle = LogicalTypeHandle::new(unsafe { duckdb_array_type_child_type(self.0.ptr) });
        LogicalType::from(handle)
    }
}
...
impl From<duckdb_logical_type> for LogicalTypeHandle {
