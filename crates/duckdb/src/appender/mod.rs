...
use crate::{
    core::{column_info::ColumnInfo, DataChunk, DataChunkHandle, LogicalType, LogicalTypeHandle},
...
impl Appender<'_> {
    ...
    pub(super) fn new(conn: &Connection, app: ffi::duckdb_appender) -> Appender<'_> {
        ...
        let column_types = (0..column_count)
            .map(|i| {
                let handle = LogicalTypeHandle::from(unsafe { ffi::duckdb_appender_column_type(app, i) });
    ...
    /// # use duckdb::{Connection, Result, params};
    ///   use arrow::record_batch::RecordBatch;
    /// fn append_rows_example(conn: &Connection, records: &[RecordBatch], expand_struct: bool) -> Result<()> {
    ///     let mut app = conn.appender("foo")?;
    ///     app.append_rows_arrow(records, expand_struct)?;
    ///     Ok(())
    /// }
    /// ```
    #[cfg(feature = "appender-arrow")]
